...
async def admin_risghts(_, CallbackQuery):
    ...
    if command == "stopcb":
        ...
        await CallbackQuery.message.reply_text(
            f"🎧 Sesli Sohbet Tarafından Sonlandırıldı {CallbackQuery.from_user.mention}!",
            reply_markup=audio_markup2,
        )
        await CallbackQuery.message.delete()
    ...
    if command == "skipcb":
        ...
        if Queues.is_empty(chat_id):
            await remove_active_chat(chat_id)
            await CallbackQuery.message.reply_text(
                f"Daha fazla müzik  __Yok__ \n\nSesli Sohbetten Ayrılıyorum.. Tarafından Kullanılan buton :- {CallbackQuery.from_user.mention}"
            )
            await Yukki.pytgcalls.leave_group_call(chat_id)
            await CallbackQuery.message.delete()
            await CallbackQuery.answer(
                "Atladı. Sırada daha fazla müzik yok", show_alert=True
            )
            return
        else:
            ...
            if str(finxx) != "raw":
                await CallbackQuery.message.delete()
                await CallbackQuery.answer(
                    "Atlandı! Çalma listesi....", show_alert=True
                )
                mystic = await CallbackQuery.message.reply_text(
                    f"**{MUSIC_BOT_NAME} Çalma listesi İşlevi**\n\n__Bir Sonraki Müziği Çalma Listesinden İndirildi....__\n\nTalep eden :- {CallbackQuery.from_user.mention}"
                )
                (
                    title,
                    duration_min,
...
async def play_playlist(_, CallbackQuery):
    ...
    if smex == "third":
        ...
        if CallbackQuery.from_user.id != int(user_id):
            return await CallbackQuery.answer(
                "Bu sizin için değil! Oyunun kendi çalma listesi", show_alert=True
            )
        _playlist = await get_playlist_names(user_id, type)
    ...
    if not _playlist:
        ...
        await CallbackQuery.message.delete()
        mystic = await CallbackQuery.message.reply_text(
            f"Çalma Listesi Başlatılıyor. {third_name}.\n\nTarafından İstendi:- {CallbackQuery.from_user.first_name}"
        )
        ...
        if for_p == 1:
            ...
            if await isPreviewUp(preview):
                await CallbackQuery.message.reply_photo(
                    photo=preview,
                    caption=f"Bu Sıraya Alınmış Çalma Listesi {third_name}.\n\nTarafından oynatıldı :- {CallbackQuery.from_user.mention}",
                    quote=False,
                    reply_markup=InlineKeyboardMarkup(buttons),
                )
                await m.delete()
            ...
            await CallbackQuery.message.reply_text(
                "Çalma Listesinde sadece 1 Müzik var.. Sıraya eklenecek daha fazla müzik yok."
            )
        if for_t == 1:
            await start_timer(
                send_video,
...
async def group_playlist(_, CallbackQuery):
    ...
    if not _count:
        sex = await CallbackQuery.message.reply_text(
            f"Hoşgeldin {MUSIC_BOT_NAME}'un Çalma Listesi Özelliğine.\n\nÇalma Listenizi Veritabanında Oluşturma...Lütfen bekleyiniz.\n\nGenre:- {genre}"
        )
    ...
    if count == 50:
        return await CallbackQuery.answer(
            "Özür dilerim! Bir çalma listesinde yalnızca 50 müzik olabilir.",
            show_alert=True,
        )
    loop = asyncio.get_event_loop()
    await CallbackQuery.answer()
    ...
    return await CallbackQuery.message.reply_text(
        f"Eklendi {type}' {genre} Çalma listesine {CallbackQuery.from_user.mention}"
    )
...
async def check_playlist(_, CallbackQuery):
    ...
    if not _playlist:
        return await CallbackQuery.answer(
            f"Sunucularda {genre} Çalma listesi yok. Çalma listesine müzik eklemeyi deneyin.",
            show_alert=True,
        )
    else:
        j = 0
        ...
        m = await CallbackQuery.message.reply_text("Çalma Listesini Çöp Kutusuna Yapıştırma")
        link = await paste_queue(msg)
        preview = link + "/preview.png"
        url = link + "/index.txt"
        ...
        if await isPreviewUp(preview):
            await CallbackQuery.message.reply_photo(
                photo=preview,
                caption=f"Çalma Listesidir. {user_name}.",
                quote=False,
                reply_markup=InlineKeyboardMarkup(buttons),
            )
            await m.delete()
...
async def del_playlist(_, CallbackQuery):
    ...
    if not _playlist:
        return await CallbackQuery.answer(
            "Grubun Botun Sunucusunda Çalma Listesi yok", show_alert=True
        )
    else:
        await CallbackQuery.message.delete()
        await CallbackQuery.answer()
    ...
    await CallbackQuery.message.reply_text(
        f"Başarıyla silindi {type}'un {genre} tüm çalma listesi\n\nTarafından :- {CallbackQuery.from_user.mention}"
    )
