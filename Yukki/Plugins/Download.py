...
__MODULE__ = "Genel Ban"
__HELP__ = """

**Not:**
Sadece Yardımcı Kullanıcıları için.

/gban [Kullanıcı adı veya kullanıcıya yanıt]
- Bot'un Sunduğu Sohbetlerde bir kullanıcıyı genel olarak yasaklayın ve kullanıcının bot komutlarını kullanmasını engelleyin.

/ungban [Kullanıcı adı veya kullanıcıya yanıt]
- Bir kullanıcıyı Botun GBan Listesinden kaldırın.
"""
    ...
    buttons = [
        [
            InlineKeyboardButton(
                text="İndirme veya Yükleme Başarısız oldu......", callback_data=f"down"
            )
        ],
        [
            InlineKeyboardButton(
                text="⬅️  Geri Dön", callback_data=f"good {videoid}|{user_id}"
            ),
            InlineKeyboardButton(
...
⚡️ __Youtube Satır İçi İndirme {MUSIC_BOT_NAME} Tarafından Desteklenmektedir__"""
...
def p_mark(link, channel):
    buttons = [
        [InlineKeyboardButton(text="Youtube'da İzle", url=f"{link}")],
    ]
    return buttons
...
async def send_file(
    ...
    await CallbackQuery.edit_message_text(
        "Yükleme Başladı\n\nYükleme hızı yavaş olabilir. Dayan lütfen..",
        reply_markup=upl,
    )
    try:
...
async def downloadvideocli(command_to_exec):
    ...
    filename = t_response.split("Birleştirme biçimleri içine")[-1].split('"')[1]
    return filename
