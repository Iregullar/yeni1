...
**Winamp Çalışma Süresi:** {uptime}
**Sistem :** Online
**Platform:** {sc}
**Mimari:** {arch}
**Ram:** {ram}
**Python Yayı:** {pyver.split()[0]}
**Pyrogram Yayı:** {pyrover}"""
...
**Depolama Kullanılabilirliği:** {total[:4]} GiB 
**Kullanılan Depolama:** {used[:4]} GiB
**Depolama Alanı Kaldı:** {free[:4]} GiB"""
        await CallbackQuery.edit_message_text(smex, reply_markup=stats3)
    if command == "bot_stats":
        await CallbackQuery.answer("Bot İstatistiklerini Almak...", show_alert=True)
        served_chats = []
...
**Banlanan Kullanıcılar:** {blocked}
**Yetkili Kullanıcıları:** {j}
**Servis Edilen Sohbetler:** {len(served_chats)}"""
    ...
    if command == "mongo_stats":
        ...
        try:
            ...
            print(e)
            return await CallbackQuery.edit_message_text("Mongo DB istatistikleri alınamadı", reply_markup=stats5)
        try:
            db = pymongo.Yukki
        except Exception as e:
            print(e)
