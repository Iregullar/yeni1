...
async def play(_, message: Message):
    ...
    if audio:
        ...
        file_name = path.join(path.realpath("indirilenler"), file_name)
        file = await convert(
        ...
        if len(message.command) < 2:
            ...
            await message.reply_photo(
                ...
                caption=(
                    "**Kullanım:** /play [Müzik Adı veya Youtube Bağlantısı veya Sese Cevap Ver]\n\nAşağıdan birini seçin."
                ),
                reply_markup=InlineKeyboardMarkup(buttons),
            )
            return
...
async def startyuplay(_, CallbackQuery):
    ...
    if str(duration) == "Yok":
        return await CallbackQuery.answer(
            f"Üzgünüm! Bu Canlı bir Video.", show_alert=True
        )
...
async def search_query_more(_, CallbackQuery):
    ...
    if CallbackQuery.from_user.id != int(user_id):
        return await CallbackQuery.answer(
            "Kendi Müziğini Ara. Bu düğmeyi kullanmana izin verilmiyor.",
            show_alert=True,
        )
